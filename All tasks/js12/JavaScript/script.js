...
function smilify(message) {
  for (const key in smiles) {
    let newString = "";
    while (
      message !==
      (newString = message.replace(
        `:${key}:`,
        `<img src=https://www.webfx.com/tools/emoji-cheat-sheet/${smiles[key]}/>`
      ))
    ) {
      message = newString;
    }
...
for (const key in smiles) {
  ...
  img.setAttribute(
    "src",
    `https://www.webfx.com/tools/emoji-cheat-sheet/${smiles[key]}`
  );
  img.setAttribute("name", `:${key}:`);
  smiles_box.appendChild(div).append(img);
  img.onclick = () => {
...
send.onclick = () => {
  sendAndCheck();
  /*
  //----1-st old request and adding to DOM
  jsonPost("http://students.a-level.com.ua:10012", {
    func: "addMessage",
    nick: `${nick.value}`,
    message: `${smilify(message.value)}`,
  });
  ...
  jsonPost("http://students.a-level.com.ua:10012", {
    ...
    tempId = r.nextMessageId;
    for (const iterator of r.data) {
      let span = document.createElement("span");
      chat_window.appendChild(span).innerHTML = `${nick.value}: ${
        iterator.message
      } time: ${hours}:${minutes < 10 ? "0" + minutes : minutes}</br>`;
    }
  });
*/
  message.value = "";
...
let tempId = 0;
let counter = 0; // just for checking checkLoop - func
async function getMessages() {
  let r = await jsonPost("http://students.a-level.com.ua:10012", {
    func: "getMessages",
    messageId: tempId,
  });
  tempId = r.nextMessageId;
  for (const iterator of r.data) {
    let hours = new Date(iterator.timestamp).getHours();
    let minutes = new Date(iterator.timestamp).getMinutes();
    let span = document.createElement("span");
    chat_window.appendChild(span).innerHTML = `${iterator.nick}: ${
      iterator.message
...
async function sendAndCheck() {
  try {
    Promise.all([sendMessage(nick.value, message.value), getMessages()]);
  } catch (err) {
    console.log(err);
  }
}
